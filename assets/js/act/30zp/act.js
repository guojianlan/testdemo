...
p.init = function() {
	...
	$(document).ready(function() {
				...
				if (ret === 2) {
					...
					switch (parseInt(respdata.data.num, 10)) {
						case 0:
							...
							break;
						case 2:
							$('#jiangpin').attr('src', './img/award0' + 4 + '.png?rf=1');
							break;
						case 4:
							$('#jiangpin').attr('src', './img/award0' + 3 + '.png?rf=1');
							break;
						case 6:
		...
		for (var i = 0; i < 8; i++) {
			var rotatedeg = (defaultreg + (i * 45)) + 23 + 'deg';
			deg.push(rotatedeg);
		}
		//0是一等奖，2是四等奖，4是三等奖，6是二等奖
		$('.zhizhen').one('click', function() {
			$.ajax({
				url: '/active-lotteryoppo/activity',
				type: 'POST',
				dataType: 'json',
				data: {},
				success: function(datajson) {
					switch (parseInt(datajson.ret, 10)) {
						case 0:
							alert('请登录后重试');
							location.href = 'http://m.lmbang.com/user/login?return_uri=http://ad.lmbang.com/html/30zp/activity.html';
							break;
						case 2:
							$('.zhuanpan').animate({
...
p.postMessage = function(username,phone,address) {
	$.ajax({
		url: '/active-lotteryoppo/saveuserinfo',
		type: 'POST',
		dataType: 'json',
		data: {
			username: username,
